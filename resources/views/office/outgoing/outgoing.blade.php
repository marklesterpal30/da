...
@section('content')
	<div class="p-2">
		...
		<form action="{{ url('/office-outgoing') }}" method="POST" enctype="multipart/form-data" class="flex flex-col space-y-14 rounded-md bg-white p-12 shadow-xl">
			@csrf
			<!------FILE UPLOAD------>
			...
			<div class="flex items-start justify-between">
				...
				<div id="INPUTS" class="flex w-full flex-col items-center justify-center rounded-md bg-green-50 p-2 sm:w-1/2">
					<div class="w-full p-2">
						<label for="file_name" class="text-xl font-semibold text-gray-600">Subject/Title/Description</label>
						<input type="text" name="file_name" class="block w-full rounded-sm border-2 border-green-400 bg-green-200" required>
					</div>
					<div class="w-full p-2">
						<label for="forward_to" class="text-xl font-semibold text-gray-600">Forward to</label>
						<input name="forward_to" type="text" class="block w-full rounded-sm border-2 border-green-400 bg-green-200" required>
					</div>
				</div>
			</div>
			<hr>

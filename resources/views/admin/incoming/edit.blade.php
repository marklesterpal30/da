	...
	<div class="h-full w-fit md:w-full">
		<form action="{{ url('/file/' . $file->id) }}" method="POST" class="flex h-full flex-col space-y-3 rounded-md bg-gray-100 p-2 shadow-md md:p-6">
			...
			<div class="grid h-full grid-cols-1 gap-x-3 gap-y-2 md:grid-cols-3">
				<div class="h-full rounded-md bg-white p-2 shadow-lg md:col-span-2">
					...
					<div class="ml-3">
						...
						@if ($file->fowarded_date)
							...
							<div class="ml-2 flex space-x-3">
								...
								<div>
									<h1 class="mb-6 w-fit rounded-md bg-green-200 px-4 py-2 text-xl font-bold">Forwarded To: <span class="font-semibold">{{ $file->forwarded->name }}</span></h1>
								</div>
							</div>
						@endif

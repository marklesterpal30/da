...
@section('content')
	<div class="h-fit w-fit rounded-md bg-white p-4 shadow-xl md:w-full">
		<h1 class="mb-4 text-2xl font-bold text-gray-600">Active Document Lists</h1>
				...
				<tbody>
					@foreach ($$category as $document)
						<tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
							...
							<td class="px-6 py-4">
								{{ $document->recieved_date ?? $document->fowarded_date }}
							</td>
							<td class="px-6 py-4">{{ $document->address_from }}</td>
							<td class="px-6 py-4">{{ $document->file_name }}</td>
							<td class="px-6 py-4">{{ $document->active_years }}</td>
							<td class="px-6 py-4">{{ $document->inactive_years }}</td>
							<td class="px-6 py-4">{{ $document->location }}</td>
							<td class="flex space-x-2 px-6 py-4">
								<a href="{{ url('/admin-alldocuments/' . $document->id . '/edit') }}" class="font-medium text-blue-600 hover:underline dark:text-blue-500"><i class="fa-solid fa-eye" style="color: #2977ff;"></i></a>
							</td>
						</tr>
					@endforeach
				</tbody>

...
@section('content')
	<div class="rounded-md bg-white p-2 shadow-md sm:p-12">
		...
		<div class="flex sm:space-x-12">
			...
			<form action="{{ url('/admin-profile') }}" method="POST" class="w-full space-y-3 bg-green-50 sm:w-1/2">
				@csrf
		...
		<div class="mt-8 flex sm:space-x-12">
			...
			<div class="w-full space-y-3 bg-green-50 sm:w-1/2">
				<form action="{{ url('/admin-profile') }}" method="POST" onsubmit="validatePasswords(event)">
					@csrf

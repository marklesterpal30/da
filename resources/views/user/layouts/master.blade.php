...
<body>
	...
	<aside id="default-sidebar" class="fixed left-0 top-14 z-40 h-screen w-64 -translate-x-full transition-transform sm:top-16 sm:translate-x-0" aria-label="Sidebar">
		<div class="h-full overflow-y-auto bg-white px-3 py-4 dark:bg-gray-800">
			<ul class="space-y-2 font-medium">
				...
				<li class="{{ request()->is('user-dashboard') ? 'bg-green-800' : 'bg-green-600' }} mb-4 rounded-md p-2 text-xl font-semibold text-white hover:bg-green-500">
					<a href="/user-dashboard" class="flex h-full w-full items-center">
						<i class="fa-solid fa-gauge mr-2" style="color: #FFFF00;"></i>
						<span class="menu-text">Dashboard</span>
					</a>
				</li>
				<li class="{{ request()->is('user-incoming') ? 'bg-green-800' : 'bg-green-600' }} mb-4 rounded-md bg-green-600 p-2 text-xl font-semibold text-white hover:bg-green-500">
					<a href="/user-incoming" class="flex h-full w-full items-center">
						<i class="fa-solid fa-file-arrow-up mr-2 text-white"></i>
						<span class="menu-text">Incoming<span class="ml-2 rounded-lg bg-white px-3 py-0.5 text-green-600">{{ $userIncomingCount }}</span></span>
					</a>
				</li>
				<li class="{{ request()->is('user-outgoing') ? 'bg-green-800' : 'bg-green-600' }} mb-4 rounded-md bg-green-600 p-2 text-xl font-semibold text-white hover:bg-green-500">
					<a href="/user-outgoing" class="flex h-full w-full items-center">
						<i class="fa-solid fa-file-arrow-up mr-2 text-white"></i>
						<span class="menu-text">Outgoing</span>
					</a>
				</li>
				<li class="{{ request()->is('user-mydocuments') ? 'bg-green-800' : 'bg-green-600' }} mb-4 rounded-md bg-green-600 p-2 text-xl font-semibold text-white hover:bg-green-500">
					<a href="/user-mydocuments" class="flex h-full w-full items-center">
						<i class="fa-solid fa-file mr-2 text-white"></i>
						<span class="menu-text">My Documents</span>
					</a>
				</li>
				<li class="{{ request()->is('user-profile') ? 'bg-green-800' : 'bg-green-600' }} mb-4 rounded-md bg-green-600 p-2 text-xl font-semibold text-white hover:bg-green-500">
					<a href="/user-profile" class="flex h-full w-full items-center">
	...
	<div class="mt-16 sm:ml-64">
		<!-- CONTENT -->
		<div class="content {{ request()->is('dashboard') ? 'p-0' : 'p-6' }} h-screen w-full overflow-y-auto rounded-md bg-green-100 shadow-lg">
			@yield('content')
		</div>
	</div>

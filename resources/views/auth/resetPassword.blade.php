...
<body>
	...
	@endif
	<div style="background-image: url('{{ asset('storage/images/loginbg.jpg') }}');" class="relative h-screen w-screen overflow-hidden bg-cover">
		...
		<div class="mx-auto flex h-screen w-screen flex-row-reverse items-center justify-center px-6 py-8 lg:py-0">
			<div class="z-50 w-full rounded-lg bg-white opacity-95 shadow dark:border dark:border-gray-700 dark:bg-gray-800 sm:w-1/3 xl:p-0">
				<div class="space-y-4 p-6 sm:p-8 md:space-y-6">
					...
					<form id="verifyForm" class="" action="{{ url('/resetPassword') }}" method="POST" autocomplete="off">
						@csrf
						<input type="text" name="email" id="email" value="{{ $email }}" class="hidden">
						<label for="newPassword">Enter your new Password</label>
						<input type="text" name="newPassword" class="w-full rounded-md bg-green-200 p-2.5">

						<button type="submit" id="submitBtn"
							class="hover:bg-primary-700 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 mt-4 w-full rounded-lg bg-green-600 px-5 py-2.5 text-center text-sm font-medium text-white opacity-95 focus:outline-none focus:ring-4">
							Reset Password
						</button>

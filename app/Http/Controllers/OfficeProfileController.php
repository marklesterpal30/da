...
class OfficeProfileController extends Controller
{
    ...
    {
        $userId = Auth::id();
        return view('office.profile.index', compact(
            'userId'    
        ));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->filled('currentPassword') && $request->filled('newPassword')) {
            if (!Hash::check($request->input('currentPassword'), $user->password)) {
                return redirect('/office-profile')->with('error', 'Current password is incorrect.');
            }
        ...
        return redirect('/office-profile')->with('success', 'Successfully updated profile.');
    }
